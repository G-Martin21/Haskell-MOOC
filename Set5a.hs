...
totalPrice (MkShoppingEntry _ value pieces) = value * (fromIntegral pieces)
...
buyOneMore (MkShoppingEntry item value pieces) = MkShoppingEntry item value (pieces + 1)
...
data UpDown = UpMode Int | DownMode Int
...
rgb Blue = [0,0,1]
rgb col = case col of
   Mix x y  -> zipWith avg (rgb x) (rgb y)
   Invert x -> zipWith (-) [1,1,1] (rgb x)
   where avg x y = (x+y)/2

-- old code
-- rgb col = case col of
--   Mix x y  -> meanList (rgb x) (rgb y)
--   Invert x -> zipWith (-) [1,1,1] (rgb x)

-- meanList :: [Double] -> [Double] -> [Double]
-- meanList xs ys = [ (x + y) / 2 | (x, y) <- zip xs ys ]
...
toList Empty = []
toList (Pair k v z) = (k,v):toList z

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((k,v):xs) = Pair k v (fromList xs)    -- (x:xs) = Pair (fst x) (snd x) (fromList xs)
...
toNat z
  | z < 0     = Nothing
  | otherwise = Just (getNat z)
  where getNat 0 = Zero
        getNat n = PlusOne (getNat (n-1))

-- old code
-- getNat :: Int -> Nat 
-- getNat z
-- | z == 0 = Zero
-- | otherwise = PlusOne (getNat (z-1))
...
-- the propose solution is more efficient when making lists, better than concat:
-- prettyPrint :: Bin -> String
...
--  my solution is a bit cumbersome, not to functional!  
-- fromBin :: Bin -> Int
-- fromBin z = fst (foldr getBin (0,0) (prettyPrint z))

-- getBin :: Integral b => Char -> (Int, b) -> (Int, b)
-- getBin c (acc, ind) = ((read [c] :: Int) * 2^ind + acc, ind+1) 
...
-- https://www.tutorialspoint.com/haskell-program-to-convert-decimal-to-binary
...
--
-- the bin is in reverse order reverse (decToBin' n)
