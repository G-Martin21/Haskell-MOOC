...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if length xs < n then xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = if i >= 0 && i < (length xs) then take i xs ++ [x] ++ drop (i+1) xs
                                               else xs
...
-- isPalindrome str = if str == reverse str then True 
--                                         else False
...
safeDiv x y = if y /= 0 then Just (div x y)
                        else Nothing
...
-- safeIndex xs i = if i >= 0 && i < length xs then Just (xs !! i)
--                                             else Nothing
safeIndex xs i
  | i < 0 = Nothing
  | i >= length xs = Nothing
