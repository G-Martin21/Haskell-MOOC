...
-- buildList start count end -- Initial status
buildList st 0 end = [end]  -- End status
buildList st ct end = st : buildList st (ct-1) end
...
sums :: Int -> [Int]
sums i = buildSum 1 i 1 -- initial status start at 1, max i, step 1

buildSum :: Int -> Int -> Int -> [Int]
buildSum st top ct = if (top == ct) then [st] else st : buildSum (st+ctUp) top (ctUp)
     where ctUp = ct+1
------------------------------------------------------------------------------
...
indexDefault (x:xs) i def 
    | i < 0  = def
    | i >= 0 = getElem i 0 (x:xs) def

getElem :: Int -> Int -> [a] -> a -> a
getElem i ct [] def = def
getElem i ct (x:xs) def = if (i == ct) then x else getElem i (ct+1) xs def
------------------------------------------------------------------------------
