...
maxBy f x y
    | f x > f y = x
    | otherwise = y
   -- apply measure to both arguments
    -- return max value of the arguments
...
firstHalf xs = take halfLength xs
  where halfLength = (length xs + 1) `div` 2
...
joinToLength k l = [ x ++ y | x <- l, y <- l, length (x ++ y) == k]
...
sumRights values = sum $ map numParticipants values
  where numParticipants (Right number) = number
        numParticipants (Left a) = 0
...
-- :{
-- compose :: [a -> b] -> a -> [b]  => [a -> a] -> a -> [a]
-- compose fs x = map ($ x) fs
-- :}
