...
-- Just define like this, we got an order enumeration
-- data Country = Finland | Switzerland | Norway
...
-- more in line with functional programming, data goes from one function to another
-- transform the list to Int, map price xs (apply price to the elements of xs), sum the result
-- sum -> map price -> xs
-- instance Price a => Price (Maybe a) where
--   price Nothing = 0
...
-- simplify p = todo
simplify (RationalNumber a b) = RationalNumber (div a divisor) (div b divisor)
